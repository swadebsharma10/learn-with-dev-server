...
async function run() {
  try {
    ...
    const courseCollection = client.db("LearnWithDevDB").collection("courses");
...
app.get('/', (req, res) => {
  res.send('Learn with dev is Sunning')
})

app.listen(port, () => {
  console.log(`Learn with dev is Sunning on port ${port}`)
})